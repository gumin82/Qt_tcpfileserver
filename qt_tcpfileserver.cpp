...
Qt_tcpfileserver::Qt_tcpfileserver(QWidget *parent)
    : QDialog(parent)
{
    totalBytes = 0;
    byteReceived = 0;
...
Qt_tcpfileserver::~Qt_tcpfileserver() {}
void Qt_tcpfileserver::start()
{
    ...
    {QMessageBox::StandardButton ret=QMessageBox::critical(this,
                                 tr("迴圈"),
                                 tr("無法做動伺服器:1.").arg(tcpServer.errorString()),
                                 QMessageBox::Retry |QMessageBox::Cancel);
            if (ret == QMessageBox::Cancel)
            return;
    }
    serverStatusLabel->setText(tr("監禮中..."));
}
void Qt_tcpfileserver::acceptConnection()
{
    tcpServerConnection= tcpServer.nextPendingConnection();//取得已接受的客戶端連線
    connect (tcpServerConnection, SIGNAL(readyRead()),
    this, SLOT (updateServerProgress()));//連按readyRead() 訊號至updateServer Progress()槽函數
    connect (tcpServerConnection, SIGNAL(error(QAbstractSocket::SocketError)),
    this, SLOT (displayError(QAbstractSocket::SocketError)));//連接error()訊號至displayError()
    serverStatusLabel->setText(tr("按受連線"));
    tcpServer.close(); //暫停接受客戶端連線
}
void Qt_tcpfileserver::updateServerProgress()
{
...
void Qt_tcpfileserver::displayError(QAbstractSocket::SocketError socketError)
{
